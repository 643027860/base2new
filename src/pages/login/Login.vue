<template>
  <div class="myLogin">
    ...
    <h1 class="bigTtitle noSelect pc">海创管理后台系统</h1>
    <div class="loginDivPc pc">
      <div class="formContain signInContainer">
        <el-form :model="loginForm" status-icon ref="loginForm" label-width="0px" class="demo-ruleForm">
          ...
          <el-form-item label="" prop="userName">
            <el-input type="text" v-model="loginForm.userName" placeholder="账户" autocomplete="new-password">
      ...
      <div class="formContain signUpContainer">
        <el-form :model="registerForm" status-icon ref="registerForm" label-width="0px" class="demo-ruleForm">
          <h1>用户注册</h1>
          <el-form-item label="" prop="userName">
            <el-input type="text" v-model="registerForm.userName" placeholder="账户" autocomplete="new-password">
              <i slot="prefix" class="el-input__icon el-icon-user"></i>
            </el-input>
          </el-form-item>
          <el-form-item label="" prop="password">
    ...
    <div class="loginDivMobile mobile">
      <div class="loginInner" v-show="loginMobile">
        <h1 class="bigTtitle noSelect">海创管理后台系统</h1>
        <p class="loginP">欢迎回来</p>
        <el-form :model="loginForm"  ref="loginForm" label-width="50px">
          <el-form-item label="账户"  prop="username">
            <el-input  prefix-icon="el-icon-s-custom" v-model.number="loginForm.username" autocomplete="off" placeholder="请输入账号"><</el-input>
          </el-form-item>
      ...
      <div class="loginInner" v-show="!loginMobile">
        ...
        <el-form :model="registerForm"  ref="registerForm" label-width="50px">
          <el-form-item label="账户"  prop="username" style="margin-bottom: 20px;">
            <el-input  prefix-icon="el-icon-s-custom" v-model.number="registerForm.username" autocomplete="off" placeholder="请输入账号"><</el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //用户登录
      Login(){
        var {userName,password} = this.loginForm
        if(password && userName){
          if(password){
            this.resertForm('loginForm')
          }else{
            var loginError = "账户密码不正确"
            this.loginForm.password = ''
            this.message(loginError,'error',1500)
          }
        }else{
          var loginError = "请输入账户密码"
          this.message(loginError,'warning',1500)
        }
      },
      //显示注册组件，修改对应的样式
      toRegister(){
      ...
      Register(mobile=false){
        var {userName,password,password1} = this.registerForm
        if(password && userName && password1){
          if(password === password1){
            ...
            var registerError = "两次密码不相同"
            this.registerForm.password1 = ''
            this.message(registerError,'error',1500)
          }
        }else{
          var registerError = "请输入完整信息"
          this.message(registerError,'warning',1500)
        }
      },
      //重置注册表单
      resertForm(formName){
